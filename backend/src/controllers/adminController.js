...
export async function updateUser(req,res){
  if(String(req.user.sub)===String(req.params.id)&&req.body.role==='customer')throw new AppError('You cannot remove your own admin access',400,'SELF_ROLE_CHANGE');
  if(req.body.role==='customer'){
    const adminCount=await User.countDocuments({role:'admin'});
    if(adminCount<=1)throw new AppError('Cannot demote the last admin',400,'LAST_ADMIN');
  }
  const {name,email,phone,company,role,interest}=req.body
  const user=await User.findByIdAndUpdate(req.params.id,{$set:{name,email,phone,company,role,interest}},{new:true,runValidators:true}).select('-passwordHash');
...
export async function updateResource(req,res){const resource=req.params.resource;const Model=models[resource];if(!Model)throw new AppError('Unknown resource',404,'NOT_FOUND');const item=await Model.findByIdAndUpdate(req.params.id,{$set:req.body},{new:true,runValidators:true,strict:true});if(!item)throw new AppError('Resource not found',404,'NOT_FOUND');publish(resource,'updated',String(item._id));res.json({success:true,item})}
...
export async function upsertSetting(req,res){const key=req.params.key;const allowedKeys=new Set(['brand','theme']);if(!allowedKeys.has(key))throw new AppError('Unknown setting key',400,'INVALID_KEY');const item=await SiteSettings.findOneAndUpdate({key},{key,value:req.body.value},{new:true,upsert:true,setDefaultsOnInsert:true});publish('settings','updated',key);res.json({success:true,setting:item})}
