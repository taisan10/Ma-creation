...
export async function createOrder(req, res) {
  ...
  // Guard against buying the same plan twice. Checked against the logged-in
  // user's account first, and against the email on the checkout form too --
  // this also catches a guest re-buying with an email that already has a
  // verified purchase before ever registering/logging in.
  const ownershipQuery = {
    plan: plan._id,
    status: 'paid',
    $or: [
      ...(req.user?.sub ? [{ user: req.user.sub }] : []),
      { email }
    ]
  }
  const existing = await Payment.findOne(ownershipQuery)
  if (existing) throw new AppError('You have already purchased this plan', 409, 'ALREADY_PURCHASED')
  ...
  try {
    ...
    if (err?.statusCode === 400 && err?.error?.code === 'DUPLICATE_ORDER') {
      throw new AppError('A payment is already in progress for this plan', 409, 'ALREADY_PURCHASED')
    }
    throw err
  }
...
export async function verifyPayment(req, res) {
  if (!env.razorpayKeySecret) throw new AppError('Razorpay secret is not configured', 503, 'PAYMENTS_NOT_CONFIGURED')
  ...
  if (sigBuf.length !== expBuf.length || !crypto.timingSafeEqual(expBuf, sigBuf)) throw new AppError('Payment signature verification failed', 400, 'INVALID_SIGNATURE')
...
export async function webhook(req, res) {
  ...
  try {
    if (event === 'payment.captured' && entity?.order_id) {
      await Payment.findOneAndUpdate({ razorpayOrderId: entity.order_id, status: 'created' }, { razorpayPaymentId: entity.id, status: 'paid', rawWebhook: req.body, verifiedAt: new Date() })
    }
