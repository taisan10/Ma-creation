...
export async function getSetting(req,res){const allowedKeys=new Set(['brand','theme']);if(!allowedKeys.has(req.params.key))return res.status(404).json({success:false,message:'Unknown setting key'});const setting=await SiteSettings.findOne({key:req.params.key});res.json({success:true,setting:setting||{key:req.params.key,value:{}}})}
