...
router.get('/users',asyncHandler(c.listUsers));router.patch('/users/:id',validate(z.object({name:z.string().min(2).max(80).optional(),email:z.string().email().optional(),phone:z.string().min(8).max(20).optional(),company:z.string().max(120).optional(),role:z.enum(['customer','admin']).optional(),interest:z.string().max(200).optional()})),asyncHandler(c.updateUser));router.delete('/users/:id',asyncHandler(c.deleteUser))
